...
```bash
node "$HOME/.codex/plugins/cache/local-plugins/devpost-hackathon-desktop/0.1.0/scripts/compose-response.mjs" --page learning-onboard
```

Use the composer output as the participant-facing response. End with a compact note that the next command is `$learning-scope`.
