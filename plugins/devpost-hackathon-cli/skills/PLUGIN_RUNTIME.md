...
```text
$HOME/.codex/plugins/cache/local-plugins/devpost-hackathon-cli/0.1.0
```

## Primary Interface

Chat is the primary participant interface. This is the Codex CLI plugin, so responses must stay text-only. Do not tell the participant to open a localhost page during normal operation.
...
```bash
node "$HOME/.codex/plugins/cache/local-plugins/devpost-hackathon-cli/0.1.0/scripts/compose-response.mjs" --page resources
```

Do not attempt to detect or switch to Desktop mode here; the visual Desktop experience lives in the separate `devpost-hackathon-desktop` plugin.
