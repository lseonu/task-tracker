...
Fallback: if `submit_project` fails on auth or availability, do NOT silently succeed — degrade in one line per `references/plugin-runtime.md`: report that the submit could not be completed, note that submitting requires being signed in to the Devpost MCP, and offer the browser handoff (open Devpost, copy `devpost-submission.md`, complete the official form). Leave `submission.status` as `ready` so the MCP submit can be retried with `$submission`.
