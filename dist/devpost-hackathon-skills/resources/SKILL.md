...
Compose the response in-context per `references/plugin-runtime.md` ("Composing the Response"): read `references/content/steps/resources.md`, strip maintainer `<!-- -->` comments, interpolate the event name, then present a short stage headline, the page content, and the next-command callout. Do not run any script.
