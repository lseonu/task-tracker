...
- `references/content/steps/help.md` (the page content you will present)
...
Do not call `devpost` MCP tools for a plain help response — orientation needs no official data. Only if the user's question also asks about dates, rules, or other event specifics, follow **Devpost MCP Server** in `references/plugin-runtime.md` and call just what that question needs.
...
Compose in-context per `references/plugin-runtime.md` ("Composing the Response"): read `references/content/steps/help.md`, strip maintainer `<!-- -->` comments, interpolate the event name, and present it as the participant-facing response — adjusted per **Tailoring** above. Trimming sections that are irrelevant to what the user asked is fine; do not add features or invent commands that are not in the page content.
