...
3. If the registration tools fail on auth or availability, degrade in one line per `references/plugin-runtime.md`: note that registering from Codex requires being signed in to the Devpost MCP, and offer the browser fallback (the official event landing page from live data or `config/hackathon.json`).
